...
const startGame = async () => {
    ...
    function showToast(message) {
        ...
        toast.style.cssText = `
            ...
            background: #333; color: #fff; padding: 14px 28px; border-radius: 50px;
            font-size: 15px; z-index: 10001; opacity: 0; transition: all 0.4s ease;
    ...
    function showConfirmModal(title, message, onConfirm) {
        ...
        overlay.innerHTML = `<div style="background:#fff;width:340px;border-radius:24px;padding:28px;text-align:center;transform:scale(0.9);transition:all 0.3s ease;box-shadow:0 20px 40px rgba(0,0,0,0.3);">
            <div style="font-size:22px;font-weight:700;margin-bottom:16px;">${title}</div>
            <div style="font-size:16px;color:#666;line-height:1.5;margin-bottom:30px;">${message}</div>
            <div style="display:flex;gap:16px;">
                <button id="modal-cancel" style="flex:1;padding:14px;border-radius:16px;border:2px solid #e0e0e0;background:#f8f9fa;color:#333;font-weight:600;cursor:pointer;">Отмена</button>
                <button id="modal-confirm" style="flex:1;padding:14px;border-radius:16px;background:var(--primary);color:#fff;font-weight:600;cursor:pointer;border:none;box-shadow:0 4px 12px rgba(96,46,171,0.3);">Подтвердить</button>
            </div>
        </div>`;
    ...
    function setupAdminControls() {
        nodes.endGameBtn.onclick = () => {
            showConfirmModal('Завершить игру', 'Игра будет завершена для всех игроков', async () => {
    ...
    function listenToGameState() {
        onSnapshot(doc(db, 'active_sessions', currentLobbyCode), (snap) => {
            if (!snap.exists() || snap.data().status !== 'playing') {
                if (snap.data()?.status === 'finished') showFinalResults();
                return;
            }
    ...
    function startWaitingTimer() {
        ...
        let waitSeconds = 5;
        nodes.waitingSeconds.textContent = waitSeconds;
