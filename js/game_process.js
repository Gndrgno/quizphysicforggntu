...
const startGame = async () => {
    ...
    const nodes = {
        ...
        statusTextPlayer: document.getElementById('statusTextPlayer'),
    ...
    let timeLeft = 10; // Время на вопрос
    let halfTime = timeLeft / 2;
    let playerPoints = 0; // Локальный счёт для игрока
    let startTime = null;
    ...
    async function initGame() {
        ...
        if (isHost) {
            ...
            nodes.statusTextPlayer.textContent = 'Игра в процессе';
    ...
    function setupPlayerControls() {
        // Логика клика на ответ в renderQuestionForPlayer
    }
    ...
    function listenToGameState() {
        ...
        // Для админа: прослушка ответов на текущий вопрос
        if (isHost) {
    ...
    // --- СТАРТ НОВОГО ВОПРОСА (только админ) ---
    ...
    // --- РЕНДЕР ТЕКУЩЕГО ВОПРОСА (для всех) ---
            ...
            btn.onclick = async () => {
                if (timeLeft > 0) {
                    ...
                    await setDoc(doc(db, 'active_sessions', currentLobbyCode, 'answers', myPlayerId), {
                        playerId: myPlayerId,
                        playerName: 'Игрок', // Можно взять из players.name
    ...
    // --- ТАЙМЕР (синхронизирован по startTime) ---
    function startTimer() {
        clearInterval(timerInterval);
        const now = Date.now();
        timeLeft = Math.max(0, Math.floor((startTime + 10000 - now) / 1000)); // 10 сек от startTime
    ...
    // --- ПОКАЗ ПРАВИЛЬНЫХ/НЕПРАВИЛЬНЫХ ОТВЕТОВ (с анимацией fill) ---
    ...
    // --- ПОКАЗ ФИНАЛЬНЫХ РЕЗУЛЬТАТОВ (для всех) ---
