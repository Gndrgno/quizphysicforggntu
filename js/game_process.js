...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const nodes = {
    ...
    leaveQuizBtn: document.getElementById('leaveQuizBtn'),
    // modals
    ...
    gameEndedModal: document.getElementById('gameEndedModal'),
    leaveEndedBtn: document.getElementById('leaveEndedBtn')
  };
  // session & user info
  ...
  function showFinalUI(playersDocs) {
    ...
    arr.forEach((p, idx) => {
      ...
      item.innerHTML = `<div style="display:flex;flex-direction:column">
                        ...
                        <div class="score">${p.score} баллов</div>${trophy}`;
      nodes.finalScoresList.appendChild(item);
    });
    if (isHost) {
      nodes.backToHomeBtn.style.display = 'inline-flex';
      nodes.leaveQuizBtn.style.display = 'none';
    } else {
      nodes.leaveQuizBtn.style.display = 'inline-flex';
      nodes.backToHomeBtn.style.display = 'none';
    }
  }
  // Render answered players for player view
  function renderAnsweredPlayers(docs) {
    nodes.answeredPlayersList.innerHTML = '';
    const answered = docs.filter(d => d.data().questionIndex === sessionState.currentQuestion && typeof d.data().selectedIndex === 'number');
    answered.forEach((d, idx) => {
      const a = d.data();
      const item = document.createElement('div');
      item.className = 'answered-player';
      item.style.animationDelay = `${idx * 0.1}s`;
      item.textContent = `${a.playerName || 'Игрок'} ответил на вопрос`;
      nodes.answeredPlayersList.appendChild(item);
    });
  }
  // ---------------------------
    ...
    function frame() {
      if (sessionState) {
        ...
        if (phase === 'answering' && qStart) {
          ...
          nodes.progressBarPlayer.style.transform = `scaleX(${pct / 100})`;
          nodes.progressBarAdmin.style.transform = `scaleX(${pct / 100})`;
          ...
          nodes.progressBarPlayer.style.transform = 'scaleX(0)';
          nodes.progressBarAdmin.style.transform = 'scaleX(0)';
          ...
          nodes.waitingProgressBarPlayer.style.transform = `scaleX(${pct / 100})`;
          nodes.waitingProgressBarAdmin.style.transform = `scaleX(${pct / 100})`;
  ...
  function subscribeAnswers() {
    ...
    answersUnsub = onSnapshot(collection(db, 'active_sessions', currentLobbyCode, 'answers'), snapshot => {
      ...
      // render answered players for player view
      if (!isHost) {
        renderAnsweredPlayers(snapshot.docs);
      }
  ...
  function setupAdminControls() {
    ...
    nodes.endGameBtn.onclick = () => {
      nodes.endGameModal.style.display = 'flex';
      nodes.endGameModal.classList.add('show');
    };
    nodes.confirmEndBtn.onclick = async () => {
      try {
        await updateDoc(doc(db, 'active_sessions', currentLobbyCode), { status: 'finished' });
        nodes.endGameModal.style.display = 'none';
        nodes.endGameModal.classList.remove('show');
        localStorage.removeItem('myPlayerId');
        sessionStorage.clear();
        window.location.href = 'homepage.html';
      } catch (e) { console.error(e); }
    };
    nodes.cancelEndBtn.onclick = () => {
      nodes.endGameModal.style.display = 'none';
      nodes.endGameModal.classList.remove('show');
    };
  }
  ...
  if (nodes.leaveQuizBtn) nodes.leaveQuizBtn.onclick = () => { localStorage.removeItem('myPlayerId'); sessionStorage.clear(); window.location.href = 'homepage.html'; };
  // player game ended modal
  nodes.leaveEndedBtn.onclick = () => { localStorage.removeItem('myPlayerId'); sessionStorage.clear(); window.location.href = 'homepage.html'; };
  // kick off
