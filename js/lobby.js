...
const startLobby = async () => {
    ...
    async function setupPlayerSession(code) {
        try {
            ...
            // === УЛУЧШЕННЫЙ БЛОК СМЕНЫ ИМЕНИ: инпут и кнопка на одной строке ===
            if (!document.querySelector('.name-change-container')) {
                const nameContainer = document.createElement('div');
                nameContainer.className = 'name-change-container';
                nameContainer.style.cssText = 'margin-bottom: 20px; width: 100%; display: flex; gap: 8px; align-items: center;';

                const nameInput = document.createElement('input');
                nameInput.type = 'text';
                nameInput.placeholder = 'Ваше имя';
                nameInput.value = playerName;
                nameInput.style.cssText = 'flex: 1; padding: 12px 16px; border-radius: 12px; border: 1px solid #e0e0e0; background: #f9f9f9; font-size: 16px; outline: none;';

                const changeNameBtn = document.createElement('button');
                changeNameBtn.textContent = '✔';
                changeNameBtn.title = 'Сменить имя';
                changeNameBtn.style.cssText = 'padding: 12px 16px; border-radius: 12px; background: #8a2be2; color: white; border: none; font-weight: bold; font-size: 18px; cursor: pointer; min-width: 48px;';
    ...
    function listenToLobby(code) {
        onSnapshot(collection(db, 'active_sessions', code, 'players'), (snapshot) => {
            ...
            snapshot.forEach((pDoc) => {
                ...
                div.style.cssText = 'display: flex; justify-content: space-between; align-items: center; width: 100%;'; // Для отступа
                div.innerHTML = `
                    <span style="font-size: 16px; font-weight: 500; color: #333;">${p.name}</span>
                    <div class="ready-status ${p.isReady ? 'is-ready' : ''}" style="margin-left: 12px;"></div>
                `;
