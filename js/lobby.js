...
const startLobby = async () => {
    ...
    async function setupPlayerSession(code) {
        try {
            ...
            // === БЛОК СМЕНЫ ИМЕНИ: инпут сверху, кнопка снизу, как у остальных кнопок ===
            if (!document.querySelector('.name-change-container')) {
                const nameContainer = document.createElement('div');
                nameContainer.className = 'name-change-container';
                nameContainer.style.cssText = 'margin-bottom: 20px; width: 100%;';

                const nameInput = document.createElement('input');
                nameInput.type = 'text';
                nameInput.placeholder = 'Ваше имя';
                nameInput.value = playerName;
                nameInput.style.cssText = 'width: 100%; padding: 14px 16px; border-radius: 14px; border: 1px solid #e0e0e0; background: #f9f9f9; font-size: 16px; outline: none; box-shadow: 0 2px 6px rgba(0,0,0,0.05); margin-bottom: 10px; box-sizing: border-box;';

                const changeNameBtn = document.createElement('button');
                changeNameBtn.textContent = 'Изменить имя';
                changeNameBtn.style.cssText = 'width: 100%; padding: 14px; border-radius: 14px; background: #8a2be2; color: white; border: none; font-weight: 600; font-size: 16px; cursor: pointer; box-shadow: 0 2px 6px rgba(138,43,226,0.3);';
                ...
                changeNameBtn.onclick = async () => {
                    ...
                    if (newName.length > 20) {
                        showToast('Имя слишком длинное (макс. 20 символов)');
                        return;
                    }
                    ...
                    try {
                        ...
                        showToast('Ошибка при смене имени');
                    }
                };
                ...
                // Вставляем блок с именем ПЕРЕД кнопкой "Я готов!"
    ...
    function listenToLobby(code) {
        onSnapshot(collection(db, 'active_sessions', code, 'players'), (snapshot) => {
            ...
            snapshot.forEach((pDoc) => {
                ...
                div.style.cssText = 'display: flex; justify-content: space-between; align-items: center; width: 100%; padding: 8px 0;';
                div.innerHTML = `
                    <span style="font-size: 16px; font-weight: 500; color: #333;">${p.name}</span>
                    <div class="ready-status ${p.isReady ? 'is-ready' : ''}" style="margin-left: 16px; flex-shrink: 0;"></div>
                `;
