...
const startLobby = async () => {
    ...
    async function setupPlayerSession(code) {
        try {
            ...
            // === БЛОК СМЕНЫ ИМЕНИ: инпут сверху, кнопка снизу ===
    ...
    function listenToLobby(code) {
        onSnapshot(collection(db, 'active_sessions', code, 'players'), (snapshot) => {
            ...
            snapshot.forEach((pDoc) => {
                ...
                nameSpan.style.cssText = 'font-size: 16px; font-weight: 500; color: #333; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 60%;';
                ...
                `;
                if (p.isReady) {
                    statusCircle.textContent = 'Готов';
                    statusCircle.style.background = '#28a745';
                } else {
                    statusCircle.textContent = 'Не готов';
