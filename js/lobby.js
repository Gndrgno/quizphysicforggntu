...
const startLobby = async () => {
    ...
    async function setupPlayerSession(code) {
        try {
            ...
            // === НОВАЯ ФУНКЦИЯ: ИНПУТ И КНОПКА ДЛЯ СМЕНЫ ИМЕНИ ===
            if (!document.querySelector('.name-change-container')) {
                const nameContainer = document.createElement('div');
                nameContainer.className = 'name-change-container';
                nameContainer.style.cssText = 'margin-bottom: 16px; width: 100%;';

                const nameInput = document.createElement('input');
                nameInput.type = 'text';
                nameInput.placeholder = 'Введите ваше имя';
                nameInput.value = playerName;
                nameInput.style.cssText = 'width: 100%; padding: 12px; border-radius: 12px; border: 1px solid #e0e0e0; background: #f9f9f9; font-size: 16px; margin-bottom: 8px; box-sizing: border-box;';

                const changeNameBtn = document.createElement('button');
                changeNameBtn.textContent = 'Сменить имя';
                changeNameBtn.style.cssText = 'width: 100%; padding: 12px; border-radius: 12px; background: #8a2be2; color: white; border: none; font-weight: 600; cursor: pointer;';
