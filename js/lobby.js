...
const startLobby = async () => {
    ...
    async function setupPlayerSession(code) {
        try {
            ...
            // === УЛУЧШЕННЫЙ БЛОК СМЕНЫ ИМЕНИ: красивая кнопка, не вылезает ===
            if (!document.querySelector('.name-change-container')) {
                const nameContainer = document.createElement('div');
                nameContainer.className = 'name-change-container';
                nameContainer.style.cssText = 'margin-bottom: 20px; width: 100%; display: flex; gap: 10px; align-items: center;';

                const nameInput = document.createElement('input');
                nameInput.type = 'text';
                nameInput.placeholder = 'Ваше имя';
                nameInput.value = playerName;
                nameInput.style.cssText = 'flex: 1; padding: 14px 16px; border-radius: 14px; border: 1px solid #e0e0e0; background: #f9f9f9; font-size: 16px; outline: none; box-shadow: 0 2px 6px rgba(0,0,0,0.05);';

                const changeNameBtn = document.createElement('button');
                changeNameBtn.textContent = 'Изменить';
                changeNameBtn.style.cssText = 'padding: 14px 20px; border-radius: 14px; background: #8a2be2; color: white; border: none; font-weight: 600; font-size: 15px; cursor: pointer; white-space: nowrap; box-shadow: 0 2px 6px rgba(138,43,226,0.3);';
