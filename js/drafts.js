document.addEventListener('DOMContentLoaded', async function () {
    ...
    const createDraftCard = (data, id) => {
        ...
        block.innerHTML = `
            ...
            <div class="card-buttons" style="display: flex; flex-direction: column; gap: 2vw;">
                <button class="add-ans edit-btn-action">Продолжить редактирование</button>
                <button class="add-ans start-btn-action" style="background: #602EAB; color: #ffffff;">Запустить викторину</button>
            </div>`;
        ...
        // Кнопка продолжения редактирования
        block.querySelector('.edit-btn-action').onclick = () => {
            sessionStorage.setItem('editQuizId', id);
            window.location.href = 'index.html';
        };

        // Кнопка запуска викторины (ИЗМЕНЕНО: теперь ведет на lobby.html)
        block.querySelector('.start-btn-action').onclick = () => {
            // Сохраняем ID викторины для использования в лобби
            sessionStorage.setItem('activeQuizId', id);
